...
jv_pool_t* jv_pool_create(jv_uint_t size)
{
    ...
    lump->size = size - sizeof(jv_lump_t) - sizeof(jv_block_t) - sizeof(jv_pool_t);
    lump->used = 0;
...
void *jv_pool_alloc(jv_pool_t *pool, jv_uint_t size)
{
    ...
    if(size > pool->max - sizeof(jv_block_t) - sizeof(jv_lump_t))
	{    /* 必须保证一个Block空闲的内存能存放jv_block_t与jv_lump_t类型的长度 */
        printf("申请的空间过大\n");
        return NULL;
    }
    ...
    for (p= pool->pos,i=0; i==0 || p!= pool->pos; i++, p = p->next)
	{
		...
		{  /* first fit */
            if(p->size <= size + sizeof(jv_lump_t))
			{   /* best fit  */
                p->used = 1;
                return (void *)(p + 1);
            }
			else
			{/* 将所找的Lump分成两个，即在Lump后面新增一个Lump，分配给所申请者 */
                jv_lump_t *lump;
...
void *jv_pool_alloc_block(jv_pool_t *pool, size_t size)
{
    ...
    u_char* cp = (u_char*)malloc(pool->max);
    if(cp==NULL)
	{
    ...
    if(size > pool->max - sizeof(jv_block_t) - 2 * sizeof(jv_lump_t))
	{    /* 新申请的block，仅生成一个lump节点,并将block的所有空闲内存分配给新节点 */
        jv_lump_t *lump;
	...
	{ /* 新申请的block，生成两个lump节点，并将后一个lump节点的size个空闲空间分配给申请者，前一个lump节点的空闲空间保留 */
        jv_lump_t *lump, *alloc;
        lump = (jv_lump_t *)(cp + sizeof(jv_block_t));
        lump->size = pool->max - sizeof(jv_block_t) - sizeof(jv_lump_t) - size - sizeof(jv_lump_t);
        lump->used = 0;

        alloc = (jv_lump_t *)((u_char *) lump + sizeof(jv_lump_t) + lump->size);
        alloc->size = size;
        alloc->used = 1;
...
jv_int_t jv_pool_free(jv_pool_t *pool, void *ptr)
{
    ...
    if ( (u_char *) idle + sizeof (jv_lump_t) + idle->size == (u_char *) idle->next)
	{ /* 与下一个节点相临 */
        if (idle->next->used == 0)
		{ /* 下一个节点空闲，则合并 */
    ...
    if ( (u_char *) prior + sizeof (jv_lump_t) + prior->size == (u_char *) idle)
	{ /* 与上一个节点相临 */
        if (prior->used == 0)
		{ /* 上一个节点空闲，则合并 */
